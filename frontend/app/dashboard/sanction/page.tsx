...
import { getSanctionLoans } from "@/lib/dashboard-api";
import type { Loan } from "@/types/loan.types";
import type { PaginatedResponse } from "@/types/dashboard.types";
import { SanctionLoansTable } from "@/components/dashboard/SanctionLoansTable";
import { ApproveModal, RejectModal } from "@/components/dashboard/SanctionActionModal";
...
export default function SanctionPage() {
  ...
  const [approveTarget, setApproveTarget] = React.useState<Loan | null>(null);
  const [rejectTarget, setRejectTarget] = React.useState<Loan | null>(null);

  React.useEffect(() => {
    if (user && !canAccessDashboardModule(user.role, "sanction")) {
  ...
  async function loadLoans(currentSearch: string, currentPage: number) {
    ...
    setError(null);
    try {
      const result = await getSanctionLoans({ search: currentSearch || undefined, page: currentPage, limit: 20 });
      setData(result);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Sanction Review" description="Review and approve or reject applied loan applications." />
